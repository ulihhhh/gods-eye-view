...
- `src/data/layerState.js` — registered `local-adsb` (token `1`,
  `enabled-only`); `REGISTERED_LAYER_IDS` count is 24 in
  `layerState.test.mjs` after merging with the AEMET and liveuamap work.
