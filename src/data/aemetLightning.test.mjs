...
import {
  ...
  createAemetLightningLayer,
  createAemetLightningOverlayEntry,
  normalizeAemetLightningStatus,
} from './aemetLightning.js';
...
test('createAemetLightningOverlayEntry builds a clickable thumbnail entry carrying the image slot through unchanged', () => {
  const entry = createAemetLightningOverlayEntry(FAKE_IMAGE_SLOT, { fetchedAtMs: null }, FAKE_POSITION);
  assert.equal(entry.id, AEMET_LIGHTNING_OVERLAY_SOURCE_ID);
  ...
  assert.equal(entry.title, 'AEMET LIGHTNING');
  assert.equal(entry.thumbnailWidth > 0, true);
  assert.equal(entry.thumbnailHeight > 0, true);
});

test('createAemetLightningOverlayEntry renders a much bigger, protected card when expanded', () => {
  const collapsed = createAemetLightningOverlayEntry(FAKE_IMAGE_SLOT, { fetchedAtMs: null, expanded: false }, FAKE_POSITION);
  const expanded = createAemetLightningOverlayEntry(FAKE_IMAGE_SLOT, { fetchedAtMs: null, expanded: true }, FAKE_POSITION);
  ...
  assert.equal(expanded.selected, false, 'never `selected: true` on a thumbnail — see the comment above createAemetLightningOverlayEntry\'s `selected` field for why that silently breaks sizing');
  assert.equal(expanded.title, 'AEMET LIGHTNING · CLICK TO COLLAPSE');
  assert.equal(collapsed.title, 'AEMET LIGHTNING');
});

test('createAemetLightningOverlayEntry\'s title carries a relative freshness label when a fetch time is known', (t) => {
  const now = Date.UTC(2026, 8, 12, 12, 0, 0);
  t.mock.method(Date, 'now', () => now);
  const justNow = createAemetLightningOverlayEntry(FAKE_IMAGE_SLOT, { fetchedAtMs: now - 30_000 }, FAKE_POSITION);
  assert.equal(justNow.title, 'AEMET LIGHTNING · JUST NOW');
  const minutesAgo = createAemetLightningOverlayEntry(FAKE_IMAGE_SLOT, { fetchedAtMs: now - 20 * 60_000 }, FAKE_POSITION);
  assert.equal(minutesAgo.title, 'AEMET LIGHTNING · 20M AGO');
  const hoursAgo = createAemetLightningOverlayEntry(FAKE_IMAGE_SLOT, { fetchedAtMs: now - 5 * 3600_000 }, FAKE_POSITION);
  assert.equal(hoursAgo.title, 'AEMET LIGHTNING · 5H AGO');
});

test('normalizeAemetLightningStatus validates shape and coerces missing fields safely', () => {
  assert.deepEqual(normalizeAemetLightningStatus({ hasKey: true, lastFetch: 100, stale: false }), {
    hasKey: true, lastFetch: 100, stale: false,
  });
  assert.deepEqual(normalizeAemetLightningStatus({}), { hasKey: false, lastFetch: null, stale: false });
  assert.equal(normalizeAemetLightningStatus(null), null);
});
...
test('update() with no key sets an error and never attempts to load an image', async () => {
  ...
  const layer = createAemetLightningLayer({
    overlayHost,
...
test('update() loads and publishes a new image only when lastFetch actually moves', async () => {
  ...
  try {
    ...
    assert.equal(published[1], AEMET_LIGHTNING_OVERLAY_SOURCE_ID);
    assert.deepEqual(published[3], AEMET_LIGHTNING_OVERLAY_SOURCE_OPTIONS);
...
test('a null lastFetch on the very first poll still triggers an image load (it means the proxy has never fetched, not "nothing new")', async () => {
  ...
  try {
    ...
    globalThis.fetch = async () => {
      statusCalls++;
      // First status call (pre-load): proxy hasn't fetched anything yet.
      // Second status call (post-load, re-read for the real timestamp):
      // the image request above just made the proxy populate its cache.
...
test('a failed image load sets an error without crashing', async () => {
  ...
  try {
    ...
    const ok = await layer.update();
    assert.equal(ok, false);
    assert.equal(layer.getStats().error, 'AEMET lightning image failed to load');
...
test('re-enabling after a disable redisplays an already-loaded image without a new fetch', async () => {
  ...
  try {
    ...
    layer.disable();
    assert.deepEqual(overlayHost.calls.at(-1), ['visible', AEMET_LIGHTNING_OVERLAY_SOURCE_ID, false]);
...
test('destroy() clears the overlay source and resets stats', async () => {
  ...
  try {
    ...
    layer.destroy();
    assert.equal(layer.getStats().count, 0);
    assert.deepEqual(overlayHost.calls.at(-1), ['clear', AEMET_LIGHTNING_OVERLAY_SOURCE_ID]);
  } finally {
    globalThis.fetch = originalFetch;
  }
});
...
test('network and HTTP errors from the status endpoint are reported, not thrown', async () => {
  const layer = createAemetLightningLayer({ overlayHost: fakeOverlayHost(), loadImage: async () => null });
  const originalFetch = globalThis.fetch;
  try {
    layer.init();
    layer.enable();
